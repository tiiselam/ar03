/****** Object:  StoredProcedure [dbo].[NuevoCITI_VC]    Script Date: 08/13/2015 18:14:31 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[NuevoCITI_VC]') AND type in (N'P', N'PC'))
DROP PROCEDURE [NuevoCITI_VC]
GO

/****** Object:  StoredProcedure [dbo].[NuevoCITI_VC]    Script Date: 10/22/2019 15:44:16 ******/
SET ANSI_NULLS ON
GO
...
SELECT	A.DOCNUMBR
		, A.RMDTYPAL
		, A.CUSTNMBR
		, ISNULL(TRX.DOCDATE, CC.DOCDATE) DOCDATE
		, RIGHT('000' + RTRIM(ISNULL(AW2.COD_COMP, AW3.COD_COMP)),3)  COMTYPE
		, RIGHT('00000' + REPLACE(ISNULL(SUBSTRING(A.DOCNUMBR, ISNULL(AW2.Pos_PDV, AW4.Pos_PDV), 5), '00000'),'-',''),5) PDV
		, RIGHT('00000000000000000000' + RTRIM(ISNULL(SUBSTRING(A.DOCNUMBR, ISNULL(AW2.Pos_NRO, AW4.Pos_NRO), 8), '0000')),20) NROCOMP
		, RIGHT(ISNULL(TRX.TXRGNNUM, CM.TXRGNNUM), 2) CODCOC
		, LEFT(ISNULL(TRX.TXRGNNUM, CM.TXRGNNUM), 11) NROIDENT
		, LEFT(RTRIM(ISNULL(TRX.CUSTNAME, CM.CUSTNAME)) + REPLICATE(' ', 30), 30) NOMBRE
		, ISNULL(TRX.DOCAMNT, CC.ORTRXAMT) DOCAMNT
		...
		, CASE WHEN CI.ColumnaArchivo = 3 THEN TDTTXSLS ELSE 0 END EXENTO
		...
		, MC.CURR_CODE MONEDA
		, ISNULL(MC1.XCHGRATE, 1) TCAMBIO
		, 0 ALICUOTAS
		, CONVERT(VARCHAR, ISNULL(AW.OP_ORIGIN, 0)) CODOPER
		, CASE WHEN A.RMDTYPAL > 6 THEN CC.DOCDATE ELSE CC.DUEDATE END DUEDATE 
FROM	RM10601 A
		...
		INNER JOIN
			(
			SELECT A.RPTID, A.DSCRIPTN, A.NRORDCOL, A.IDTAXOP, B.TAXDTLID, C.TXDTLDSC 
			FROM	AWLI40004 A 
					INNER JOIN AWLI40102 B ON A.IDTAXOP = B.IDTAXOP
					INNER JOIN TX00201 C ON B.TAXDTLID = C.TAXDTLID
			WHERE rptid IN(@REPORTE)	 
			--WHERE rptid IN('IVA VENTAS')	 
			) TX ON A.TAXDTLID = TX.TAXDTLID
...
WHERE ISNULL(TRX.VOIDSTTS, 0) = 0 
AND ISNULL(CC.VOIDSTTS, 0) = 0 
AND left(CONVERT(char(8), ISNULL(TRX.DOCDATE, CC.DOCDATE), 112), 6) = @PERIODO 
--AND left(CONVERT(char(8), ISNULL(TRX.DOCDATE, CC.DOCDATE), 112), 6) = '201901' 
AND CI.TipoReporte = 2
...
SELECT	A.DOCNUMBR 
		...
		, CASE WHEN CI.ColumnaArchivo = 8 THEN A.TAXAMNT ELSE 0 END PIIBB
		, MC.CURR_CODE MONEDA
		, ISNULL(MC1.XCHGRATE, 1) TCAMBIO, 0 ALICUOTAS
		, CONVERT(VARCHAR, ISNULL(AW.OP_ORIGIN, 0)) CODOPER
		, CASE WHEN A.RMDTYPAL > 6 THEN CC.DOCDATE ELSE CC.DUEDATE END DUEDATE 
FROM	RM30601 A
		...
		INNER JOIN
			(
			SELECT A.RPTID, A.DSCRIPTN, A.NRORDCOL, A.IDTAXOP, B.TAXDTLID, C.TXDTLDSC 
			FROM	AWLI40004 A INNER JOIN AWLI40102 B ON A.IDTAXOP = B.IDTAXOP
					INNER JOIN TX00201 C ON B.TAXDTLID = C.TAXDTLID
			WHERE rptid IN(@REPORTE)	 
			--WHERE rptid IN('IVA VENTAS')	 
			) TX ON A.TAXDTLID = TX.TAXDTLID
...
SELECT 	DOCNUMBR,
		...
		CUSTNMBR,
		DOCDATE,
		RIGHT('000' + COMPTYPE,3) COMTYPE,
		RIGHT('00000' + LTRIM(RTRIM(PDV)) ,5)  PDV,
		NROCOMP,
		CODDOC,
		NROIDENT,
		NOMBRE,
...
FROM	#TEMP A
		INNER JOIN 
			(
			SELECT A.DOCNUMBR, A.RMDTYPAL, A.CUSTNMBR, COUNT(A.ALICUOTAS) ALICUOTAS 
			FROM
				(
				SELECT DISTINCT A.DOCNUMBR, A.RMDTYPAL, A.CUSTNMBR, B.TXDTLPCT ALICUOTAS 
				FROM	RM10601 A 
						INNER JOIN
							(
							SELECT A.RPTID, A.DSCRIPTN, A.NRORDCOL, A.IDTAXOP, B.TAXDTLID, C.TXDTLDSC, CI.ColumnaArchivo 
							FROM	AWLI40004 A 
									INNER JOIN AWLI40102 B ON A.IDTAXOP = B.IDTAXOP
									INNER JOIN tblColumnasIVA CI ON A.RPTID = CI.RPTID AND A.NRORDCOL = CI.NRORDCOL
									INNER JOIN TX00201 C ON B.TAXDTLID = C.TAXDTLID
							WHERE A.rptid IN(@REPORTE) 
							--WHERE A.rptid IN('IVA VENTAS') 
							AND CI.TipoReporte = 2 
							AND CI.ColumnaArchivo = 2 
							) TX ON A.TAXDTLID = TX.TAXDTLID
						INNER JOIN TX00201 B ON TX.TAXDTLID = B.TAXDTLID
				WHERE TX.ColumnaArchivo = 2 AND A.TAXAMNT <> 0
				) A 
			GROUP BY A.DOCNUMBR, A.RMDTYPAL, A.CUSTNMBR 
			) TX ON A.DOCNUMBR = TX.DOCNUMBR AND A.RMDTYPAL = TX.RMDTYPAL AND A.CUSTNMBR = TX.CUSTNMBR
...
SELECT 
		...
		'000000000' + RTRIM(NROIDENT)		+				-- NRO IDENTIFICACION DEL COMPRADOR
		...
		CASE 
			WHEN CODOPER = 1 THEN 
				CASE 
					WHEN EXENTO <> 0 THEN 'E' 
					ELSE CASE 
							WHEN ALICUOTAS = 0 THEN 'N' 
							ELSE '0' 
						 END 
				END
			ELSE 
		...
		CASE 
			WHEN COMPTYPE > '008' THEN '00000000' 
			ELSE CONVERT(CHAR(8), DUEDATE, 112)	
		END	TEXTO
FROM #TEMP WHERE dbo.fncESNUMERICO(PDV) = 1 AND PDV <> '00000'
