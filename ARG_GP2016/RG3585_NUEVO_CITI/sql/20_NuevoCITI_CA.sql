/****** Object:  StoredProcedure [dbo].[NuevoCITI_CA]    Script Date: 08/13/2015 18:14:31 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[NuevoCITI_CA]') AND type in (N'P', N'PC'))
DROP PROCEDURE [NuevoCITI_CA]
GO

/****** Object:  StoredProcedure [dbo].[NuevoCITI_CA]    Script Date: 25/04/2019 14:38:03 ******/
SET ANSI_NULLS ON
GO
...
INSERT INTO #TEMP
SELECT A.VCHRNMBR, A.DOCTYPE, A.VENDORID, ISNULL(AW.AWLI_DocNumber, TRX.DOCNUMBR) DOCNUMBR, TRX.TRXDSCRN, TRX.PORDNMBR, TRX.DOCDATE
, RIGHT('000' + RTRIM(ISNULL(AW.COD_COMP, '00')),3)  COMTYPE
, CASE WHEN ISNULL(AW.COD_COMP, '00') <> '66' THEN RIGHT('00000' + ISNULL(RTRIM(SUBSTRING(AW.AWLI_DocNumber, 5,(case when CHARINDEX('-',aw.awli_docnumber) < 5 then 9 else CHARINDEX('-',aw.awli_docnumber)-1 end)-(4))), '00000'), 5) ELSE '00000' END PDV
, CASE WHEN ISNULL(AW.COD_COMP, '00') <> '66' THEN RIGHT(REPLICATE('0', 20) + ISNULL(RTRIM(SUBSTRING(AW.AWLI_DocNumber, case CHARINDEX('-',aw.awli_docnumber) when 0 then 11 else CHARINDEX('-',aw.awli_docnumber)+1 end, 8)), '0000'), 20) ELSE REPLICATE('0', 20) END NROCOMP
