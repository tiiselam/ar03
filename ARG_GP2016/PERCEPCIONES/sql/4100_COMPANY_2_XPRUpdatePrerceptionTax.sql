...
@iFileName char(100), @iFileFromDate char(8), 
...
begin  
	...
	begin  
		...
		BEGIN  
			...
			(Regimen char(1), PubDate char(8),DueDateFrm char(8),DueDateTo char(8),
			 TaxIDCode char(11),TypeTaxIDCode char(1),  StatInd char(1),StatusOfChange char(1),
			 PercPer char(4),PerCa int, Dummy char(10))      
		...
		begin  
			select * into ##XPRTemp2 from  (select [CUSTNMBR] ,[ADRSCODE] ,[TAXDTLID],[AX_Start_Date] ,  [AX_Due_Date], [AX_Tipo] , [AX_Porcentaje] ,[AX_Organismo],[AX_Certificado_Entregado] from XPR00102   where AX_Tipo=2 and TAXDTLID in (select TAXDTLID from XPR00106 where NAME=@iFileName)   and CUSTNMBR in(select RM00101.CUSTNMBR from  RM00101 where substring(TXRGNNUM,24,2)<>'' and substring(TXRGNNUM,1,11) not in(select TaxIDCode from ##XPRTemp5)and  RM00101.CUSTNMBR=XPR00102.CUSTNMBR)  and ((AX_Start_Date<@iFromDate and AX_Due_Date>=@iFromDate)  or( AX_Start_Date>@iFromDate and AX_Start_Date<@iToDate and AX_Due_Date>@iToDate)  or( AX_Start_Date>=@iFromDate and AX_Due_Date<=@iToDate)))as t1   
			if @@error<>0  begin  rollback transaction  return  end  
		end  
		if @ProcessType=1 or @ProcessType=2  
		begin  
			update XPR00102 set AX_Due_Date=DATEADD(day,-1,@iFromDate)  where AX_Tipo=2 and TAXDTLID in (select TAXDTLID from XPR00106 where NAME=@iFileName)   and CUSTNMBR in(select RM00101.CUSTNMBR from  RM00101 where substring(TXRGNNUM,24,2)<>'' and substring(TXRGNNUM,1,11) not in(select TaxIDCode from ##XPRTemp5) and RM00101.CUSTNMBR=XPR00102.CUSTNMBR)  and (AX_Start_Date<@iFromDate and AX_Due_Date>=@iFromDate)   
			if @@error<>0  begin  rollback transaction  return  end   
			update XPR00102 set AX_Start_Date=DATEADD(day,1,@iToDate)  where AX_Tipo=2 and TAXDTLID in (select TAXDTLID from XPR00106 where NAME=@iFileName)   and CUSTNMBR in(select RM00101.CUSTNMBR from  RM00101 where substring(TXRGNNUM,24,2)<>'' and substring(TXRGNNUM,1,11) not in(select TaxIDCode from ##XPRTemp5) and RM00101.CUSTNMBR=XPR00102.CUSTNMBR)  and (AX_Start_Date<=@iToDate and AX_Due_Date>@iToDate)   
			if @@error<>0  begin  rollback transaction  return  end  
			...
			if @@error<>0  begin  rollback transaction  return  end  
			delete from XPR00102  where  AX_Tipo=2 and TAXDTLID in(select TAXDTLID from XPR00106 where NAME=@iFileName)  and AX_Start_Date>=@iFromDate and AX_Due_Date<=@iToDate  and CUSTNMBR in(select RM00101.CUSTNMBR from  RM00101 where substring(TXRGNNUM,24,2)<>'' and substring(TXRGNNUM,1,11) not in(select TaxIDCode from ##XPRTemp5) and RM00101.CUSTNMBR=XPR00102.CUSTNMBR)  
			if @@error<>0  begin  rollback transaction  return  end  
		end  
		...
		BEGIN  
			if @ProcessType=1   
			begin  
				insert  into XPR00102(CUSTNMBR,ADRSCODE,TAXDTLID,AX_Start_Date,AX_Due_Date,  AX_Tipo,AX_Porcentaje)  select distinct ##XPRTemp6.CUSTNMBR,  ##XPRTemp6.ADRSCODE,##XPRTemp6.TAXDTLID,@iFromDate,@iToDate,2,CAST(REPLACE(##XPRTemp1.PercPer,',','.') AS decimal(4,2))as PercPer from ##XPRTemp6 inner join ##XPRTemp1 on substring(##XPRTemp6.TXRGNNUM,1,11)=##XPRTemp1.TaxIDCode  
				if @@error<>0  begin  rollback transaction  return  end  
				insert into XPR00102(CUSTNMBR,ADRSCODE,TAXDTLID,AX_Start_Date,AX_Due_Date,  AX_Tipo,AX_Porcentaje)  select distinct ##XPRTemp6.CUSTNMBR,  ##XPRTemp6.ADRSCODE,##XPRTemp6.TAXDTLID,@iFromDate,@iToDate,2,@DefPerc from ##XPRTemp6  where  substring(##XPRTemp6.TXRGNNUM,1,11)  not in (select ##XPRTemp1.TaxIDCode from ##XPRTemp1 where ##XPRTemp1.TaxIDCode=substring(##XPRTemp6.TXRGNNUM,1,11))  and  substring(##XPRTemp6.TXRGNNUM,1,11) not in(select TaxIDCode from ##XPRTemp5)  
				if @@error<>0  begin  rollback transaction  return  end   
			end  
			if  @ProcessType=2  
			begin  
