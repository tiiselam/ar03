...
/****** Object:  StoredProcedure [dbo].[NuevoCITI_CE]    Script Date: 04/25/2019 14:49:31 ******/
SET ANSI_NULLS ON
GO
...
UPDATE #TEMP SET ERRORES = RTRIM(ERRORES) + '/EL COMPROBANTE ' + COMPTYPE + ' NO PUEDE GENERAR CREDITO FISCAL'
WHERE CREDFISC <> 0 AND COMPTYPE NOT IN('001', '002', '003', '004')
...
SELECT 
		...
		IMPINT,
		OTROSTRIB,
		TOTAL-(GRAVADO21+GRAVADO105+GRAVADO27+NOGRAV+PERCNOCAT+EXENTO+PERCIVA+PERCNAC+PERCIIBB+PERCMUN+IMPINT+CREDFISC+OTROSTRIB) DIFERENCIA,
		MONEDA,
		CAMBIO,
		ALICUOTAS,
		CODOPER,
...
AND A.DOCTYPE < 4 AND RTRIM(ERRORES)<>''
UNION
SELECT 
		A.VCHRNMBR,
		A.DOCTYPE,
